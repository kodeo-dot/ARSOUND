-- Create or replace RPC functions for download and upload management

-- Function to get packs uploaded this month
...
BEGIN
  ...
  RETURN COALESCE(pack_count, 0);
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- Function to get remaining uploads for current month
...
BEGIN
  ...
  CASE LOWER(plan_type)
    ...
    WHEN 'studio_plus' THEN month_limit := NULL;
...
-- Function to verify download permission
...
BEGIN
  ...
  CASE LOWER(user_plan)
    ...
    WHEN 'studio_plus' THEN download_limit := NULL;
...
-- Helper function to increment counter safely
...
BEGIN
  EXECUTE format(
    'UPDATE %I SET %I = %I + 1, updated_at = NOW() WHERE id = %L',
    table_name, column_name, column_name, row_id
  );
END;
$$ LANGUAGE plpgsql;

-- Function to get download limit per plan
...
BEGIN
  ...
  month_start := DATE_TRUNC('month', NOW());
  SELECT COUNT(*) INTO current_downloads
  FROM public.pack_downloads
  WHERE user_id = p_user_id AND downloaded_at >= month_start;
  ...
  RETURN jsonb_build_object(
    ...
    'used', current_downloads,
    'remaining', GREATEST(0, download_limit - current_downloads)
  );
END;
$$ LANGUAGE plpgsql IMMUTABLE;
