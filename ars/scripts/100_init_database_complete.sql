-- Complete ARSOUND Database Initialization
-- This script ensures all tables are created with proper structure
...
CREATE INDEX IF NOT EXISTS idx_purchases_mercado_pago_id ON public.purchases(mercado_pago_payment_id);
...
DROP POLICY IF EXISTS "Users can insert their own packs" ON public.packs;
DROP POLICY IF EXISTS "Users can update their own packs" ON public.packs;

CREATE POLICY "Packs are viewable by everyone" ON public.packs FOR SELECT USING (is_deleted = false OR user_id = auth.uid());
CREATE POLICY "Users can insert their own packs" ON public.packs FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own packs" ON public.packs FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- RLS Policies for purchases
DROP POLICY IF EXISTS "Users can view their own purchases" ON public.purchases;
DROP POLICY IF EXISTS "System can insert purchases" ON public.purchases;

CREATE POLICY "Users can view their own purchases" ON public.purchases FOR SELECT USING (auth.uid() = buyer_id OR pack_id IN (SELECT id FROM public.packs WHERE user_id = auth.uid()));
